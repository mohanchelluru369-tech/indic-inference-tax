...
- Assistants working through a sandbox that mounts this folder: do not run git commands that write the index (`git status`, `git diff`, `git add`) there. The sandbox usually cannot delete files, so it leaves a `.git/index.lock` behind that blocks the next real git command. Read state with `git log`/`git show` or plain `ls`, and leave staging and committing to the human at their own terminal — that also keeps commits under the right author.
